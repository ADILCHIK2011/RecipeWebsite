...
import amerika_banner from '../assets/aqsh_banner.png'
...
const Amerika = () => {
  const [amerika, setAmerika] = useState([])
  const navigate = useNavigate()
 useEffect(()=>{
  const getAmerika = async () => {
    try {
...
getAmerika()
  ...
  return (
    <div>
      <Navbar />
      <section
        style={{ backgroundImage: `url(${amerika_banner})` }}
        className="bg-no-repeat bg-cover bg-center w-full h-[450px]"
      ></section>
      <Category/>
      <h1 className="text-center text-5xl text-base-content font-bold my-6">AQSH</h1>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 justify-items-center  px-4">
        {amerika
          ?.filter(food => food.category === "american")
          .map((food) => (
