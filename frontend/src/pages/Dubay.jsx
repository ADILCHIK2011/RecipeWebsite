...
import r from '../assets/dubay_banner.png'
...
const Dubay = () => {
  const [dubay, setDubay] = useState([])
  const navigate = useNavigate()

 useEffect(()=>{
  const getDubay = async () => {
    try {
...
getDubay()
  ...
  return (
    <div>
      ...
      ></section>
    <Category/>
      <h1 className="text-center text-5xl text-base-content font-bold my-6">Dubay</h1>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 justify-items-center  px-4">
        {dubay
          ?.filter(food => food.category === "dubai")
          .map((food) => (
