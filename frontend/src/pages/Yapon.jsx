...
import yapon_banner from '../assets/yapon_banner.png'
...
const Yaponiya = () => {
  const [yaponiya, setYaponiya] = useState([])
  const navigate = useNavigate()

 useEffect(()=>{
  const getYaponiya = async () => {
    try {
...
getYaponiya()
  ...
  return (
    <div>
      <Navbar />
      <section
        style={{ backgroundImage: `url(${yapon_banner})` }}
        className="bg-no-repeat bg-cover bg-center w-full h-[450px]"
      ></section>
    <Category/>
      <h1 className="text-center text-5xl text-base-content font-bold my-6">YAPONIYA</h1>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 justify-items-center  px-4">
        {yaponiya
          ?.filter(food => food.category === "japanese")
          .map((food) => (
