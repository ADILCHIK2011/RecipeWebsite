...
import a from '../assets/saudiya_banner.png'
...
const Arabiston = () => {
  const [arabiston, setArabiston] = useState([])
  const navigate = useNavigate()

 useEffect(()=>{
  const getArabiston = async () => {
    try {
...
getArabiston()
  ...
  return (
    <div>
      ...
      ></section>
    <Category/>
      <h1 className="text-center text-5xl text-base-content font-bold my-6">Saudiya Arabistoni</h1>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 justify-items-center  px-4">
        {arabiston
          ?.filter(food => food.category === "saudi")
          .map((food) => (
